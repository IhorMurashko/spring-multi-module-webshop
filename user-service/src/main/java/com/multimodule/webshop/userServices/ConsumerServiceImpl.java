...
import com.multimodule.webshop.util.RandomPasswordGenerator;
...
public class ConsumerServiceImpl implements ConsumerService {
    ...
    @Override
    public Consumer findByUsername(@NonNull String username) {
    ...
    @Override
    public Consumer findById(@NonNull Long id) {
    ...
    @Transactional
    @Override
    public String resetPassword(@NonNull String username) {

        boolean isConsumerExistByUsername = consumerJpaRepository.existsByUsername(username);

        if (isConsumerExistByUsername) {

            String newConsumerPassword = RandomPasswordGenerator.generateRandomPassword();

            consumerJpaRepository.updateUserPassword(username,
                    passwordEncoder.encode(newConsumerPassword));

            return newConsumerPassword;
        } else {
            throw new ConsumerUsernameNotFoundException
                    ("consumer with username " + username + " not found");
        }
    }
}
